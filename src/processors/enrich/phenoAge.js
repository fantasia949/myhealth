...
export function getPhenotypicAge2(albumin, creatinine, glucose, cReactiveProtein, lymphocytePercent, 
                                 meanCellVolume, redBloodCellDistributionWidth, alkalinePhosphatase, 
                                 whiteBloodCellCount, age) {
    ...
    if (cReactiveProtein <= 0) {
        throw new Error("C Reactive Protein must be positive for logarithmic conversion.");
    }
    ...
    const xb = (term_albumin + term_creatinine + term_glucose + term_crp + term_lymphocyte + 
    ...
    const t_months = 120;
    const mort_score_part = 1 - Math.exp(-Math.exp(xb) * (Math.exp(gamma * t_months) - 1) / gamma);

    if (mort_score_part <= 0) {
        throw new Error("Mortality score calculation part must be positive for logarithmic conversion.");
    }

    if (1 - mort_score_part <= 0) {
        throw new Error("1 - MortScore must be positive for logarithmic conversion.");
    }

    // Calculate Phenotypic Age using Mortality Score
    const ptypic_age = 141.50225 + Math.log(-0.00553 * Math.log(1 - mort_score_part)) / 0.09165;

    const dnam_phenoage = ptypic_age / (1 + 1.28047 * Math.exp(0.0344329 * (-182.344 + ptypic_age)));
