...
exports.buildChartOption = void 0;

var _misc = require("./misc");

var buildChartOption = function buildChartOption(chartOptions, settings, chartType) {
  var _option = chartOptions || {};

  var _ref = settings || {},
      _ref$legendPosition = _ref.legendPosition,
      legendPosition = _ref$legendPosition === void 0 ? 'top' : _ref$legendPosition; // legend
  ...
  if (legendPosition) {
    ...
    switch (legendPosition) {
      ...
      default:
        ...
        _option.legend.left = 'center'; // _option.legend.right = 'center';
  ...
  var iconRect = 'path://M 0 4 A 4 4 0 0 1 4 0 L 10 0 A 4 4 0 0 1 14 4 L 14 10 A 4 4 0 0 1 10 14 L 4 14 A 4 4 0 0 1 0 10 Z';
  var iconLine = 'path://M 0 2 A 2 2 0 0 1 2 0 L 12 0 A 2 2 0 0 1 14 2 L 14 0 A 2 2 0 0 1 12 2 L 2 2 A 2 2 0 0 1 0 0 Z';
  var iconLineWithRing = 'path://M14,4h-2.1C11.4,1.7,9.4,0,7,0S2.6,1.7,2.1,4H0v2h2.1c0.5,2.3,2.5,4,4.9,4s4.4-1.7,4.9-4H14V4z M7,8  C5.3,8,4,6.7,4,5s1.3-3,3-3s3,1.3,3,3S8.7,8,7,8z';

  if (chartType == 'pie') {
    _option.legend.data = _option.series[0] && _option.series[0].data.map(function (item) {
      var textStyle = {
        padding: [2, 0, 0, 0]
      };
      return {
    ...
    var _legendData = (_option$series = _option.series) === null || _option$series === void 0 ? void 0 : _option$series.map(function (item) {
      var _item$data, _item$data$, _item$data$$itemStyle;

      var icon = 'circle';
      var textStyle = {
        padding: [2, 0, 0, 0] // color: 'rgba(0,0,0,0.85)',
        // fontWeight: 'lighter',

      };
      var itemStyle;

      if ((_item$data = item.data) !== null && _item$data !== void 0 && (_item$data$ = _item$data[0]) !== null && _item$data$ !== void 0 && (_item$data$$itemStyle = _item$data$.itemStyle) !== null && _item$data$$itemStyle !== void 0 && _item$data$$itemStyle.color) {
        itemStyle = {
    ...
    if (_option.legend.show && legendPosition && ['left', 'right'].includes(legendPosition) && (chartType == 'stack' || chartType == 'line-stack')) {
      ...
      _option.tooltip.formatter = function (data) {
        return "<div>".concat(data[0].name, "</div>").concat(data.reverse().map(function (d) {
          switch (d.seriesType) {
            case 'pie':
              return "<div>\n                        <span style=\"display:inline-block;margin-right:5px;border-radius:10px;width:9px;height:9px;background-color:".concat(d.color, "\"></span>\n                        <span>").concat(d.data.label ? d.data.label.formatter(d, true) : d.data.value, "</span>\n                    </div>");

            default:
              return "<div>\n                        <span style=\"display:inline-block;margin-right:5px;border-radius:10px;width:9px;height:9px;background-color:".concat(d.color, "\"></span>\n                        <span>").concat(d.seriesName, ":</span>\n                        <span>").concat(d.data.label ? d.data.label.formatter(d, true) : d.data.value, "</span>\n                    </div>");
          }
        }).join(''));
      };
    } else {
  ...
  _option.legend.formatter = function (text) {
    if (isNaN(text)) {
      return (0, _misc.truncate)(text, 12);
    } else {
      return text;
    }
  }; // tooltip
