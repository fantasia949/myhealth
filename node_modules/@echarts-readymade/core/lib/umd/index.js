(function (global, factory) {
  ...
  (global = typeof globalThis !== 'undefined' ? globalThis : global || self, factory(global.EchartsReadymadeCore = {}, global.React));
  ...
  const echartsOptionsBase = {
    ...
    tooltip: {
      ...
      formatter: function(data) {
        ...
        return `<div style="color: #c8c8c8;">${_data[0].name}</div><div style="column-count: ${_count};">${_data.map((d) => {
        switch (d.seriesType) {
          case "pie":
            return `<div>
                      ...
                      <span style="color: #262626;">${d.data.isShowPercentValue ? d.percent + "%" : d.data.value == null ? "--" : d.data.isPercent ? numberWithCommas(d.data.value) + "%" : numberWithCommas(Big(d.data.value).toFixed(d.data.decimalLength != null ? d.data.decimalLength : 2))}</span>
                  </div>`;
          case "line":
            return `<div>
                      <span style="display:inline-block;margin-right:5px;border-radius:10px;width:9px;height:9px;background-color:${d.color}"></span>
                      <span style="color: #595959;">${d.seriesName}:</span>
                      <span style="color: #262626;">${d.data.value == null ? "--" : d.data.isPercent ? d.data.value != null ? `${numberWithCommas(d.data.value)}%` : `--` : numberWithCommas(Big(d.data.value).toFixed(d.data.decimalLength != null ? d.data.decimalLength : 2))}</span>
                  </div>`;
          default:
            return `<div>
                      <span style="display:inline-block;margin-right:5px;border-radius:10px;width:9px;height:9px;background-color:${d.color}"></span>
                      <span style="color: #595959;">${d.seriesName}:</span>
                      <span style="color: #262626;">${d.data.value == null ? "--" : d.data.isPercent ? numberWithCommas(d.data.value) + "%" : numberWithCommas(Big(d.data.value).toFixed(d.data.decimalLength != null ? d.data.decimalLength : 2))}</span>
                  </div>`;
        }
      }).join("")}</div>`;
      }
  ...
  const mergeOption = (baseOptions, userOptions = {}) => {
    ...
    const _mergeOptions = __spreadValues(__spreadValues({}, _baseOptions), _userOptions);
  ...
  const ChartContext = React__default["default"].createContext({});
  ...
  const ChartProvider = (props) => {
    ...
    return React__default["default"].createElement(Provider, { value: { data, echartsOptions, echartsOptionsBase, userOptions } }, props.children);
  };
